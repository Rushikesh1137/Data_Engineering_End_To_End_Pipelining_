...
.load(f"abfss://olisdata@datalakeawstoragefor.dfs.core.windows.net/bronze/olist_customers_dataset.csv")

display(customer_df)
#hey here is my data is kept, now we are in the container

# COMMAND ----------

# MAGIC %md
# MAGIC #Reading_the_Data
...
# .option("header", "true")\
# .option("inferSchema", True)\
# .load(f"abfss://olisdata@datalakeawstoragefor.dfs.core.windows.net/bronze/olist_geolocation_dataset.csv")
...
# .option("header", "true")\
# .option("inferSchema", True)\
# .load(f"abfss://olisdata@datalakeawstoragefor.dfs.core.windows.net/bronze/olist_order_items_dataset.csv")

#automating all csv file

# Define the base path
base_path = "abfss://olisdata@datalakeawstoragefor.dfs.core.windows.net/bronze/"
...
# MAGIC %md
# MAGIC ### Calculate Delivery and Time Delays
...
# MAGIC %md
# MAGIC ### Saving a  DataFrame (final_df) in Parquet format in Apache Spark.
# MAGIC This will go in the silver layer ,here we are using overwrite mode (If a file or folder already exists at the given location, delete it and write the new data. )

# COMMAND ----------

final_df.write.mode("overwrite").parquet("abfss://olisdata@datalakeawstoragefor.dfs.core.windows.net/silver")
